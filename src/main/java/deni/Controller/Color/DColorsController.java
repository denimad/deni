 ...
 */
@Deprecated
public class DColorsController 
{
	/**
	...
	public DColorsController(ControlP5 cp5, 
		Object variableObject, 
		String groupName,
		PickerType type)
	{
		...
		bangPosition= new int[]{
			DColorsController.DEFAULT_BANG_INIT_POSX,
			DColorsController.DEFAULT_BANG_INIT_POSY};
		...
		Object variableObject)
	{
		...
		Bang newBang = this.cp5.addBang(varName + "bang")
			.setSize(DColorsController.DEFAULT_BANG_WIDTH, 
					DColorsController.DEFAULT_BANG_HEIGHT)
				...
				.addCallback(new CallbackListener()
					{
						public void controlEvent(CallbackEvent event) 
						{
							...
							int bangColor = DColorsController.this.colors.get(bangName);
							float alfa = event.getController().getValue();
							
							DColorsController.this.setBangColors(DColorsController.this.colorBangs.get(bangName), 
								DColorHelper.getInstance().getColorWithAlpha(bangColor, 
									alfa));
						}
					});
	...
	private void setBangColors(Bang bang, 
		int color)
	{
		float alfa = DColorHelper.getInstance().getAlpha(color)-50;
		int activeColor = DColorHelper.getInstance().getColorWithAlpha(color, alfa);
			...
			this.cp5.addColorWheel(bangVarName)
				...
				.addCallback(new CallbackListener()
				{
					public void controlEvent(CallbackEvent event) 
					{
						...
						{
							DColorsController.this.currentModifiedBang.setColorForeground(newColor);
							DColorsController.this.setBangColors(currentModifiedBang, newColor);
							DColorsController.this.cp5.remove(event.getController().getName());
